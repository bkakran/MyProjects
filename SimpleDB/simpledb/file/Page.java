...
public class Page {
   ...
   public synchronized void setString(int offset, String val) {
	...
	/**
	 * Returns the short value at the specified offset of the page. If a short
	...
	/**
	 * Returns the boolean value at the specified offset of the page. If a boolean
	 * was not stored at that location, the behavior of the method is
	 * unpredictable.
	 *
	 * @param offset the offset
	...
	public synchronized boolean getBoolean(int offset) {
		...
		byte[] byteval = new byte[len];
		contents.get(byteval);
		String flag = new String(byteval);
	...
	public synchronized void setBoolean(int offset, boolean val) {
		  ...
		  byte[] byteval = flag.getBytes();
	      contents.putInt(byteval.length);
	      contents.put(byteval);
	}

	/**
	 * Returns the byte value at the specified offset of the page.
	 *
	 * @param offset the offset
	 * @return the bytes
	 */
	...
	/**
	 * Writes a byte to the specified offset on the page.
	 *
	 * @param offset the offset
	 * @param val
	 */
	...
	/**
	 * Returns the Date value at the specified offset of the page. If a Date
	 * was not stored at that location, the behavior of the method is
	 * unpredictable.Remember Date is stored as String in dd-MM-yy:HH:mm:SS format.
	 *
	 * @param offset the offset
	 * @return the date
	 */
	public synchronized Date getDate(int offset) {
		...
		byte[] byteval = new byte[len];
		contents.get(byteval);
		String dateString = new String(byteval);
		Date date =null;
		SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("dd-MM-yy:HH:mm:SS");
        try {
        	date = DATE_FORMAT.parse(dateString);
		} catch (ParseException e) {
		 System.out.println("Failed Reading of date");
		 return null;
		}
        return date;
	}
	...
	public synchronized void setDate(int offset, Date val) {
		SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("dd-MM-yy:HH:mm:SS");
		String date = DATE_FORMAT.format(val);
		contents.position(offset);
		byte[] byteval = date.getBytes();
		contents.putInt(byteval.length);
		contents.put(byteval);
	}
}
